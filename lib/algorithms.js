...
const exponentiation = (b, e, context) => {
    if(context.n === 0) {
	return Math.pow(b, e);
    }
    let result = b;
    for(let i=1;i<e;i++) {
	result = context.multiply(result, b);
    }
    return result;
}
...
const searchGenerators = (context) => {
    ...
    return `${generators.length} full generators (primitive roots) found: {${generators.join(", ")}}`;
}
